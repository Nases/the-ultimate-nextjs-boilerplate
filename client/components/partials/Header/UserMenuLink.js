...
export default ({ children, href, isMobile = false }) => {
  ...
  if (isMobile) {
    ...
    return (
      <Link href={href}>
        <a className={`${router.pathname == href ? 'cursor-default border-primary text-primary' : 'border-transparent hover:border-gray-300 text-gray-700 hover:bg-gray-100'} block border-l-2 px-4 py-2 text-sm leading-5 transition duration-150 ease-in-out`}>
          {children}
        </a>
      </Link>
    )
