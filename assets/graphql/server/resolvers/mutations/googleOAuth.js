...
const googleOAuth = async (parent, { googleID, email, firstName, lastName }, { req, res }, info) => {
  return schema.validate({ googleID, email, firstName, lastName }).then(values => {
    return User.findOne({ email }).then(user => {
      if (!user) {
        return User({ email, googleID, firstName, lastName }).save().then(user => {
